...
; time variables
Clock  equ es:6Ch 
enemyArray dw 40 dup(0);contains all 6 Enemies x y of the players ?,?,?,?,?,?,?,?,?,?,?,?
rand dw ?
...
push si
	...
	cmp [index], 0 ;if the program is able to perform the shot then create the shot else jump to the end
	jne endCreateLaser

	cmp [counter], 4 ;if there is more than 4 laser mid game then jump to the end else create a shot
	jge endCreateLaser
	...
	inc [counter] ;increase shots counter
...
push [laserY]
	...
	dec [counter] ;decrease the laser counter by 1
...
PROC rightx
	cmp [spaceShipx], 305 ;if spaceship is in the border wont let it move towards the border else the spaceship will be able to move toward the pressed key
	jge rightButton
...
PROC random
	...
	and al, 200 ; leave result between 0-200
	mov ah, 00000100b
	mov [rand], ax
	add [rand], 100
	pop ax
